...
export function validateTutorialTips(model) {
  ...
  for (const node of entries) {
    ...
    tip.pages.forEach((page, index) => {
      ...
      if ((['enemyIntent', 'enemy'].includes(page.position?.anchor) || page.highlightEnemy || page.highlightEnemyBars?.length) && !tip.enemyState && !tip.event) pageIssue('敵を基準にする場合はenemyStateまたはeventが必要です。');
