...
- 画像差し替え用の子Container名は `card-art-slot`。現在は図形の装飾を配置しており、この層を入れ替えてもカードの当たり判定・説明文・動作は維持できる。外枠も同ファイルから変更できる。
- 説明欄の有効幅は130px、高さは86px。日英の名称は105×34pxに収め、拡大プレビュー用のテキストは2倍解像度で描画する。
- 手札は浅い扇形（最大±5度）に並び、未選択時は右側のカードが前面になる。ホバー中のカードは角度を戻して1.12倍に拡大し、画面内へ持ち上げる。隣接カードは距離に応じて左右へ退く。持ち上げ中も元の下端付近で選択が途切れないよう当たり判定を補い、ホバー解除は65ms待ってカード間移動の揺れを抑える。用語Tipsの待機は従来どおり300ms。
- 通常ドローは左の山札から65ms間隔で弧を描いて手札へ移動する。状態異常などからの追加はプレイヤー位置で光を散らしてから手札へ移動する。全追加演出が終わるまで手札入力をロックする。
- 敵対象カードは対象へ移動して効果を適用し、自分側のカードは使用位置から少し持ち上げて発動する。使用後は右の捨て札へ縮小しながら移動し、束が反応する。消滅/一時カードは上へ薄れながら光を散らし、捨て札に加えない。通常の捨て札・消滅・ターン終了時破棄のゲーム処理は既存の定義に従う。
- `src/ui/cardMotion.ts` が手札の姿勢・弧の移動・発光演出を担当する。移動TweenはカードContainerを対象とし、ホバーや使用への移行時に既存Tweenを安全に置き換える。
- 山札/捨て札の束をクリックすると `src/ui/pileBrowser.ts` の確認画面を開く。1ページ8枚、右側に大きな詳細プレビュー、コスト/名前順の並べ替え、前後ボタン・左右キー・ホイールによるページ移動、Esc/画面外クリックで閉じる操作に対応する。枚数が増えても画面外にはみ出さず、並べ替えは表示用コピーにのみ適用する。山札の実際のドロー順は公開しない。
- 確認画面を開いている間はカード使用・敵選択・ターン終了を受け付けない。閉じる時にキー/ホイールのイベントを解除する。
