...
`src/data/tutorialTips.ts` の `TUTORIAL_TIPS` に文章（日英）、イベント戦闘ID、表示条件、位置、強調対象をまとめる。通常Tipsと同じクレヨン描画・改行・文字量に応じた枠サイズを使い、背景を白・文字を黒にする。チュートリアルTipsはさらに上下左右へ各0.5文字分の背景余白を追加し、枠内クリック判定も拡張後のサイズに合わせる。

- `eventBattleId` が現在のイベント戦闘と一致する定義のみ有効。各 `id` はその戦闘で1回だけ表示する。リトライ時は記録を初期化する。
