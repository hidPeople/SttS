...
登場中は初期ターンのフック・開始時通知・ドローを待ち、End Turnは無効。レティクルも登場後に表示する。PhaserのScene Tweenを使うのでCtrl早送りの対象となる。完了時やScene終了時に倍率・透明度・マスクを復元し、Scene終了で待機中の初期ターン処理も中止する。見た目・テンポはユーザーが確認する。
