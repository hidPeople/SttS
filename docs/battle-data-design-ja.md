...
ラン中に持ち越す `player.epPeakByPart` と `player.epDamageByPart` を使い、正のEPダメージを記録した直後に条件を満たす最大レベルへ更新します。Peakが起きなくてもダメージ条件で上昇します。複数部位への攻撃では各部位に記録ダメージ全量を加算し、Peak時は各部位の回数も1ずつ増やす既存の集計仕様を維持します。ツールの「状態異常」→「PART_SENSITIVITY_LEVELS」→レベル番号から条件・倍率を編集できます。レベル数は既存の状態ID・型に合わせた1～5です。
