...
- 山札/捨て札のテキストをクリックすると `src/ui/pileBrowser.ts` の確認画面を開く。左は0.56倍のカードを8列で詰めた連続スクロール一覧（3行24枚以上）、右は大きな詳細プレビュー。ページ分けは行わず、ホイール・上下キー・スクロールバーで移動する。Home/Endで先頭/末尾へ移動できる。コスト/名前順の並べ替え、Esc/画面外クリックで閉じる操作に対応する。表示領域外は描画と当たり判定を両方切り取り、並べ替えは表示用コピーにのみ適用する。山札の実際のドロー順は公開しない。
