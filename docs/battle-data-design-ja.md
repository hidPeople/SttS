...
- 開始時フック・会話・カード追加・ドロー・カード使用の解決が終わり、プレイヤーが操作可能になった時点で条件を確認する。複数成立時は配列の上から1つずつ表示し、閉じた後に残りの条件を再確認する。通常戦闘には表示しない。
...
- `position.anchor` は `endTurn`（ボタンの上）、`card`（`cardId` の手札右上）、`enemyIntent`（対象敵の行動予告上）、`screen`（固定位置）。`x/y` はアンカーからの補正px、screen時はTips左下の画面座標。枠は画面内に収める。対象カードがまだ手札にない場合は表示済みにせず待つ。
- `highlightCardId` は一致する手札全枚、`highlightEnemy` は条件に合う対象敵のSpriteを暗転の前に出す。表示中は敵Spriteのアニメーションを一時停止し、元々再生中だったものだけ再開する。
- Tips外クリックで閉じ、内側クリックでは閉じない。背面のカード・ボタン・敵選択にはクリックを通さない。キーボードはEnter/Z/Escapeで閉じられる。閉じる・Scene終了のどちらでも深度とアニメーションを復元する。

初期データは、1ターン目の終了案内、3ターン目の誘惑追加後の使用案内、初回挿入時の引き抜く案内、初回の敵Peak余韻案内の4件。文章内の `{player}` などは既存の表示文字置換を使う。表示ロジックは `models/tutorialTips.ts`、モーダル表示と復元は `ui/tutorialTips.ts` に分離する。見た目・クリックによる確認はユーザーが担当する。
